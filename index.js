...
// ==================== AUDIO TRANSCRIBE ====================
// NOTE: ffmpeg is NOT used here — Whisper API accepts the audio directly from Cloudinary URL.
app.post("/api/transcribe", upload.single("file"), async (req, res) => {
  try {
    ...
    // Write to /tmp (Vercel allows temp writes during function execution)
    const tempOutput = path.join("/tmp", `audio_${Date.now()}.mp3`);
    fs.writeFileSync(tempOutput, audioBuffer);

    const form = new FormData();
    form.append("file", fs.createReadStream(tempOutput), { filename: "audio.mp3" });
...
app.post("/api/user/:email/skills", async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email });
    if (!user) return res.status(404).json({ success: false, message: "User not found" });
    if (user.skills.includes(skill.trim())) return res.json({ success: true, message: "Skill already exists" });
...
app.delete("/api/user/:email/skills/:index", async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email });
    if (!user) return res.status(404).json({ success: false, message: "User not found" });
    if (index < 0 || index >= user.skills.length) return res.status(400).json({ success: false, message: "Invalid skill index" });
    user.skills.splice(index, 1);
    await user.save();
    return res.json({ success: true, message: "Skill deleted successfully", skills: user.skills });
  } catch (err) {
...
app.get("/api/user-by-email/:email", async (req, res) => {
  try {
    const user = await User.findOne({ email: req.params.email }).select("firstName lastName email image role");
    if (!user) return res.status(404).json({ message: "User not found" });
    res.json(user);
  } catch (err) {
    res.status(500).json({ message: "Server error" });
...
function validateSignupPayload(payload) {
  const errors = [];
  if (!payload.firstName || String(payload.firstName).trim().length < 2) errors.push("First name is required (min 2 characters).");
  if (!payload.lastName || String(payload.lastName).trim().length < 1) errors.push("Last name is required.");
  if (!payload.phone || !/^\+?[0-9]{7,15}$/.test(String(payload.phone).trim())) errors.push("Phone is required (digits only, 7-15 chars, optional leading +).");
  if (!payload.email || !validator.isEmail(String(payload.email))) errors.push("A valid email is required.");
  if (!payload.password || String(payload.password).length < 6) errors.push("Password is required (min 6 characters).");
  if (!payload.role || !["Labour", "Contractor"].includes(payload.role)) errors.push("Role must be either 'Labour' or 'Contractor'.");
  return errors;
}
...
function signJwt(user) {
  ...
  return jwt.sign({ sub: user._id.toString(), email: user.email, role: user.role }, secret, { expiresIn });
}
...
app.post("/api/signup", async (req, res) => {
  try {
    ...
    return res.status(201).json({
      user: { id: user._id, firstName: user.firstName, lastName: user.lastName, phone: user.phone, email: user.email, role: user.role, createdAt: user.createdAt },
      token,
    });
  } catch (err) {
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    const token = signJwt(user);
    return res.json({ user: { id: user._id, firstName: user.firstName, lastName: user.lastName, email: user.email, phone: user.phone, role: user.role }, token });
  } catch (err) {
    return res.status(500).json({ error: "Internal server error." });
  }
});
          ...
          <div style="font-family:'Segoe UI',sans-serif;background:#f5f7fa;padding:40px 0;">
            <div style="max-width:600px;background:#fff;margin:0 auto;border-radius:12px;overflow:hidden;box-shadow:0 4px 12px rgba(0,0,0,.1);">
              <div style="background:#0a66c2;padding:25px 20px;text-align:center;">
                <img src="${logoUrl}" alt="Labour Hub" width="70" height="70" style="border-radius:50%;border:2px solid #fff;margin-bottom:10px;">
              ...
              <div style="padding:30px 25px;color:#333;">
                ...
                <p>Dear <strong>${user.email}</strong>,<br><br>Your Labour Hub account password has been changed successfully.</p>
              ...
              <div style="background:#f0f2f5;text-align:center;padding:20px;border-top:1px solid #e1e4e8;">
                <p style="color:#777;font-size:13px;margin:0;">&copy; ${new Date().getFullYear()} Labour Hub. All rights reserved.<br>Karachi, Pakistan</p>
              </div>
            </div>
          </div>`,
      });
    } catch (emailErr) {
      console.error("Email send failed:", emailErr.response ? emailErr.response.body : emailErr);
    }
...
app.get("/api/user/:id", async (req, res) => {
  try {
    ...
    res.json({
      ...
      role: user.role || "",
      email: user.email || "",
      image: user.image?.trim() || DEFAULT_IMAGE,
    });
  } catch (err) {
    res.status(500).json({ message: "Server error" });
  }
...
app.post("/api/jobs/apply/:jobId", async (req, res) => {
  ...
  try {
    ...
    const alreadyApplied = job.applicants.some((app) => app.laborId && app.laborId.toString() === labourId);
...
app.post("/api/jobs", async (req, res) => {
  try {
    const { title, description, location, workersRequired, skill, budget, contact, startDate, endDate, createdBy } = req.body;
    if (!title || !description || !location || !workersRequired || !skill || !budget || !contact || !startDate || !endDate)
      return res.status(400).json({ message: "All fields are required." });
    const job = new Job({ title, description, location, workersRequired, skill, budget, contact, startDate, endDate, createdBy });
    await job.save();
...
app.get("/api/profile/:email", async (req, res) => {
  try {
    const { email } = req.params;
    const user = await User.findOne({ email: email.trim().toLowerCase() }).select("firstName lastName role email image createdAt reviews").lean();
    if (!user) return res.status(404).json({ message: "User not found" });
...
app.get("/api/jobs/user/:email", async (req, res) => {
  try {
    ...
    for (const app of jobApplications) {
      ...
      if (!job) continue;
      jobsApplied.push({
        jobId: job._id, title: job.title, status: "pending", appliedAt: app.appliedAt,
        contractor: { firstName: job.createdBy.firstName, lastName: job.createdBy.lastName, email: job.createdBy.email, role: job.createdBy.role, image: job.createdBy.image || DEFAULT_IMAGE },
      });
    }
...
app.get("/api/responses-by-contractor/:email", async (req, res) => {
  try {
    ...
    if (!applications || applications.length === 0) return res.status(404).json({ message: "No responses found" });
    ...
    for (const app of applications) {
      ...
      results.push({
        applicationId: app._id, jobId: job._id, jobTitle: job.title, jobDescription: job.description,
        location: job.location, workersRequired: job.workersRequired, appliedAt: app.appliedAt,
        labour: { labourId: labour?._id || null, firstName: labour?.firstName || "Unknown", lastName: labour?.lastName || "Unknown", email: labour?.email || app.labourEmail, role: labour?.role || "Labour", image: labour?.image || DEFAULT_IMAGE },
      });
    }
...
app.get("/api/search-jobs", async (req, res) => {
  try {
    ...
    const query = {};
    if (skill?.trim()) query.skill = { $regex: new RegExp(skill, "i") };
    if (name?.trim()) query.title = { $regex: new RegExp(name, "i") };
    const jobs = await Job.find(query).sort({ createdAt: -1 });
    return res.json({ success: true, count: jobs.length, jobs });
  } catch (err) {
    res.status(500).json({ success: false, message: "Server error while searching jobs" });
  }
});
...
app.post("/api/industries/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: industry._id, email: industry.email }, process.env.JWT_SECRET, { expiresIn: "7d" });
...
app.post("/api/borrow", async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Borrow email failed:", emailErr.message);
    }
  } catch (err) {
...
<html><head><title>Labour Hub | Admin Panel</title><meta charset="UTF-8"/>
<style>
...
<h2>👤 Users</h2>
<table><tr><th>Name</th><th>Email</th><th>Phone</th><th>Role</th><th>Skills</th><th>Created</th></tr>
${users.map((u) => `<tr><td>${u.firstName} ${u.lastName}</td><td>${u.email}</td><td>${u.phone}</td><td><span class="badge blue">${u.role}</span></td><td>${u.skills?.join(", ") || "-"}</td><td>${new Date(u.createdAt).toLocaleString()}</td></tr>`).join("")}
</table>

<h2>🛠 Jobs</h2>
<table><tr><th>Title</th><th>Location</th><th>Skill</th><th>Budget</th><th>Workers</th><th>Applicants</th><th>Posted By</th></tr>
${jobs.map((j) => `<tr><td>${j.title}</td><td>${j.location}</td><td>${j.skill}</td><td>${j.budget}</td><td>${j.workersRequired}</td><td>${j.noOfWorkersApplied || 0}</td><td>${j.createdBy?.email || "-"}</td></tr>`).join("")}
</table>

<h2>📄 Job Applications</h2>
<table><tr><th>Job ID</th><th>Contractor</th><th>Labour</th><th>Date</th></tr>
${applications.map((a) => `<tr><td>${a.jobId}</td><td>${a.contractorEmail}</td><td>${a.labourEmail}</td><td>${new Date(a.appliedAt).toLocaleString()}</td></tr>`).join("")}
</table>

<h2>🏭 Industries</h2>
<table><tr><th>Industry</th><th>Owner</th><th>Email</th><th>Phone</th><th>Textile</th><th>Status</th></tr>
${industries.map((i) => `<tr><td>${i.industry}</td><td>${i.owner}</td><td>${i.email}</td><td>${i.phone}</td><td>${i.textileType}</td>
<td><form method="POST" action="/api/admin/industry-toggle/${i._id}" onsubmit="return confirm('${i.active ? "Deactivate" : "Activate"} this industry?');">
<button type="submit" class="toggle-btn ${i.active ? "green" : "red"}">${i.active ? "Active" : "Inactive"}</button></form></td></tr>`).join("")}
</table>

<h2>🔄 Borrow Requests</h2>
<table><tr><th>From</th><th>To</th><th>Labour</th><th>Skills</th><th>Location</th><th>Status</th></tr>
${borrows.map((b) => `<tr><td>${b.fromIndustryEmail}</td><td>${b.toIndustryEmail}</td><td>${b.labourRequired}</td><td>${b.skills}</td><td>${b.location}</td>
<td><span class="badge ${b.status === "Approved" ? "green" : b.status === "Rejected" ? "red" : "gray"}">${b.status}</span></td></tr>`).join("")}
</table>
