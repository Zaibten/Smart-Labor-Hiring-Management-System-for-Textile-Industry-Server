...
// NOTE: twilio is imported but only used if you add SMS features
// const twilio = require("twilio");

// NOTE: Socket.IO is NOT supported on Vercel (serverless).
// Remove it or use a separate real-time service like Pusher/Ably/Railway.

// NOTE: ffmpeg is NOT available on Vercel serverless.
// The /api/transcribe route will use a workaround (direct URL to Whisper).
...
// NOTE: /api/chat route is handled by chatRoutes below
app.use("/api/chat", chatRoutes);

// ── Transcribe (ffmpeg removed — Whisper accepts audio URLs directly)
app.post("/api/transcribe", upload.single("file"), async (req, res) => {
  try {
    ...
    // Send directly to Whisper as a Buffer (no ffmpeg needed)
...
app.post("/api/users/:email/review", async (req, res) => {
  ...
  try {
    ...
    if (user.expoPushToken) {
      notification
        .notifyUserAboutNewReview(user.expoPushToken, reviewerEmail, rating, jobTitle || null)
...
// Note: /api/user/:userId and /api/user/:id both exist — kept as-is
...
app.post("/api/manual-add-token", async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ error: "Labour user not found with this email" });
...
app.post("/api/jobs", async (req, res) => {
  try {
    ...
    const job = new Job({
      ...
      createdBy: { userId: createdBy.userId, firstName: createdBy.firstName, lastName: createdBy.lastName, role: createdBy.role, email: createdBy.email },
    });
    await job.save();
    ...
    try {
      ...
      if (labourUsers.length > 0) {
        notificationResult = await notification.notifyLabourUsersAboutNewJob(labourUsers, job);
      }
    } catch (notifError) {
      console.error("❌ New job notification error:", notifError);
    }

    return res.status(201).json({ message: "Job created successfully", job, notificationsSent: notificationResult.successCount });
  } catch (err) {
...
app.put("/api/jobs/:jobId/applicants/:labourId/status", async (req, res) => {
  ...
  try {
    ...
    if (!applicant) return res.status(404).json({ message: "Applicant not found on this job" });
    ...
    try {
      ...
      if (labour?.expoPushToken) {
        await notification.notifyLabourAboutApplicationStatus(labour.expoPushToken, job, status);
      }
    } catch (notifErr) {
...
app.get("/api/profile/:email", async (req, res) => {
  try {
    ...
    const averageRating =
      totalReviews > 0 ? (reviews.reduce((sum, r) => sum + r.rating, 0) / totalReviews).toFixed(1) : 0;
...
app.post("/api/industries/login", async (req, res) => {
  try {
    ...
    if (expoPushToken) { industry.expoPushToken = expoPushToken; await industry.save(); }

    const token = jwt.sign({ id: industry._id, email: industry.email }, process.env.JWT_SECRET || "YOUR_SECRET_KEY", { expiresIn: "7d" });
...
app.post("/api/borrow", async (req, res) => {
  try {
    ...
    if (!toIndustryEmail) { console.error("❌ toIndustryEmail missing"); return; }
    ...
    } catch (notifErr) { console.error("❌ Borrow push notification error:", notifErr); }
...
app.post("/api/approve-borrow/:id", async (req, res) => {
  try {
    ...
    } catch (notifErr) { console.error("❌ Borrow approval push error:", notifErr); }
...
app.post("/api/admin/industry-toggle/:id", async (req, res) => {
  try {
    ...
    await industry.save();
    // Redirect doesn't work well in serverless — return JSON instead
...
<tr>
  ...
  <td>
    <button onclick="toggleIndustry('${i._id}',${i.active})" class="toggle-btn ${i.active ? "green" : "red"}">
...
// ─── IMPORTANT: Export for Vercel (do NOT call app.listen) ───────────────────
module.exports = app;
